...
# PMOVES hardening: dedicated non-root runtime user (UID/GID 1000)
RUN groupadd -g 1000 -r opennotebook && useradd -u 1000 -r -g opennotebook -m -s /sbin/nologin -c "Open Notebook user" opennotebook
...
# Copy pre-downloaded tiktoken encoding from builder (outside /data/ â€” volume-mount safe)
...
# PMOVES hardening: drop privileges for the default runtime stage
USER opennotebook
